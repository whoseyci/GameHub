...
function buildDeck(spec: GameSpec, st: RngStateHolder): number[] {
  const cards: number[] = [];
...
function endRound(s: SState, spec?: GameSpec) {
...
const SPEC_BY_ID: Record<string, GameSpec> = {};
...
export const __schemaInternals = { buildDeck, deckTotalOf: (s: GameSpec) => s.deck.reduce((a, d) => a + d.count, 0) };
